...
class Board(object):
    ...
    def motor_speed(self, value):
        if self._is_connected:
            if self._motor_speed != value:
                ...
                #self._send_command("$110=500")
                #self._send_command("$111=2000")
                #self._send_command("$101=35.5")
                #self._send_command("$120=100")
                #self._send_command("$121=500")
    ...
    def motor_reset_origin(self):
        if self._is_connected:
            self.motor_move_calibration(0)
            self._motor_position = 0

    def motor_move(self, step=0, nonblocking=False, callback=None):
        if self._is_connected:
            self._motor_position += step * self._motor_direction * 2
            self.send_command("G0X{0}".format(self._motor_position), nonblocking, callback)

    def motor_move_calibration(self, step=0, nonblocking=False, callback=None):
        if self._is_connected:
            self._motor_position += step * self._motor_direction
            self.send_command("G0Y{0}".format(self._motor_position), nonblocking, callback)

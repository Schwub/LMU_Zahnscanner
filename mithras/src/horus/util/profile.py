    ...
    def _initialize_settings(self):
        ...
        self._add_setting(
            Setting('frame_rate', _('Frame rate'), 'profile_settings',
                    int, 20, possible_values=(30, 25, 20, 15, 10, 5)))
        self._add_setting(
            Setting('motor_step_control', _(u'Step (mm)'), 'profile_settings',
                    float, 90.0))
        self._add_setting(
            Setting('motor_speed_control', _(u'Speed (mm/s)'), 'profile_settings',
                    float, 200.0, min_value=1.0, max_value=1000.0))
        self._add_setting(
            Setting('motor_acceleration_control', _(u'Acceleration (mm/s²)'), 'profile_settings',
                    float, 200.0, min_value=1.0, max_value=1000.0))
        ...
        self._add_setting(
            Setting('estimated_size', _('Estimated size'), 'calibration_settings',
                    np.ndarray, np.ndarray(shape=(3,), buffer=np.array([7, 2, 90]))))
        ...
        self._add_setting(
            Setting('machine_diameter', _('Machine diameter'), 'machine_settings', int, 35))
        self._add_setting(
            Setting('machine_width', _('Machine width'), 'machine_settings', int, 35))
        self._add_setting(
            Setting('machine_height', _('Machine height'), 'machine_settings', int, 35))
        self._add_setting(
            Setting('machine_depth', _('Machine depth'), 'machine_settings', int, 35))
        # Hack to translate combo boxes:
        _('Circular')
        _('Rectangular')
        self._add_setting(
